...
\begin{homeworkProblem}
	\begin{enumerate}[(a)]
		...
		\begin{equation*}
			\begin{aligned}
				Prob(D < u)	&= Prob(e^{-a H_T} < u)	\\
							&= \int_0^u e^{-ax} \times be^{-bx} dx	\\
							&= \int_0^u be^{-(a + b)x}dx	\\
							&= \left. -\frac{b}{a + b} e^{-(a + b)x} \right \rvert^{u}_0	\\
							&= -\frac{b}{a + b} [e^{-(a + b)u} - 1]
			\end{aligned}
		\end{equation*}
		Because $L$ is proportional to $D$, cumulative distribution of $D$ is
		derived as $(1 - R) \times Prob(D < u) = -(1 - R) \frac{b}{a + b} [e^{-(a + b)u} - 1]$.
		...
		\centering
			...
			\label{fig:prob2-cdf}
		\end{figure}
		From the figure, it can be discovered that fraction of borrowers default
		and proportion of loss converges to some value. As $u$ increases,
		$e^{-(a + b)u}$ converges to zero, therefore $D$ converges to
		$\frac{b}{a + b}$ and $L$ converges to $(1 - R)\frac{b}{a + b}$.
		\item  %% (b)
		\item  %% (c)
	\end{enumerate}
\end{homeworkProblem}
