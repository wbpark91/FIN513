...
\begin{homeworkProblem}
	...
	\begin{equation*}
	\begin{aligned}
		&0.5 \times (u - 1)^2 + 0.5 \times (d - 1)^2 = 0.5 \times (u - 1)^2 + 0.5 \times (1 - u)^2 = (\frac{0.1}{12})^2	\\
		&\Rightarrow (u - 1)^2 = (\frac{1}{120})^2	\\
		&\Rightarrow u = 1 \pm \frac{1}{120}
	\end{aligned}
	\end{equation*}
	Since $u > d$, $u = 1 + \frac{1}{120} = 1.0083$ and $d = 2 - u = 0.9917$. By using the given interest rate to calculate risk-neutral probability $\bar{q}$, it is calculated as $\bar{q} = \frac{1.02 / 1.08 - 0.9917}{1.0083 - 0.9917} = -2.8467$, which is not between 0 and 1, does not satisfy the condition derived in (a). Therefore, if the value of $u$ and $d$ calculated above are used for constructing tree, arbitrage occurs.
	\end{enumerate}
\end{homeworkProblem}
...
%----------------------------------------------------------------------------------------
	...
	\end{figure}
%----------------------------------------------------------------------------------------
	Since there are 4 states at period 2, there are four possible returns and corresponding probabilities. Table \ref{tab:returns and probs} represents returns and probabilities at each states.
%----------------------------------------------------------------------------------------
	%% RETURNS AND PROBS
	\begin{table}[]
	\centering
	...
	22.34                & 0.111           & 0.2475               \\
	23.05                & 0.142           & 0.2475               \\
	6.38                 & -1.142          & 0.2025               \\ \bottomrule
	\end{tabular}
	\caption{Returns and probabilities at each state}
	\label{tab:returns and probs}
	\end{table}
	From the table, since returns and probabilities at each state are given, it is possible to calculate expected return and variance. Expected return is calculated as $\mathrm{E}(r) = \sum_{i = 1}^4 r_i p_i = 0.889 \times 0.3025 + 0.111 \times 0.2475 + 0.142 \times 0.2475 -1.142 \times 0.2025 = 10.03\%$, Variance is calculated as $\mathrm{Var}(r) = \sum_{i = 1}^4 (r_i - \mathrm{E}(r))^2 = 50.13\%$. Therefore, standard deviation is equal to $\sqrt{50.13}\% = 70.81\%$, and annualized variance and volatility is equal to $25.07\%$ and $50.07\%$, respectively.
	%%
	\item
...
%----------------------------------------------------------------------------------------
	...
	\end{figure}
	From the tree, the option value is calculated as 5.695 when volatility is not constant. In order to compare to value of option with constant volatility, stock price tree(Figure \ref{tree:prob5-stock price tree: const volatility}) is constructed using volatility calculated from (a).
%----------------------------------------------------------------------------------------
	...
	\label{tree:prob5-stock price tree: const volatility}
  	\end{figure}
	Using the same backward induction method, option price was calculated as in Figure \ref{tree:prob5-option price tree: const volatility}. The value calculated is 5.055, which is smaller than option price using non-constant volatility.
%----------------------------------------------------------------------------------------
