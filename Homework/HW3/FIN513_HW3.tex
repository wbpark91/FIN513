...
\newcommand{\hmwkDueDate}{Tuesday,\ February\ 13,\ 2018} % Due date
...
\begin{enumerate}[(a)]
	\item
	There is an arbitrage opportunity. Assuming the statistical probabilities are true, there is no chance that stock price becomes between 100 and 120. However, if a portfolio is constructed such that it has two long position of call option with strike price 110 and a long position of call option with strike 100 and 120 each(It is equivalent to sell short butterflies), it is possible to make arbitrage profit since the initial cost for constructing portfolio is negative(cash inflow of 0.25 for each portfolio) and there is no chance for cash outflow at maturity.
	\item
	The statistical probabilities assumes that there is no possibility that stock price becomes between 90 and 120. However, as mentioned in (a), market price of a claim which guarantees positive payoff when stock prices becomes between 100 and 120 is positive. It implies there is a positive (risk-neutral) probability that stock price becomes between 90 and 120. In other words, statistical probabilities and market implied probabilities differ.
	\end{enumerate}
\end{homeworkProblem}
\end{document}
